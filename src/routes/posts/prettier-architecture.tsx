  ...
  return (
    <main className="mx-auto max-w-4xl space-y-12 px-6 py-20">
      <header className="space-y-4">
        <p className="text-muted-foreground text-sm">2025년 9월</p>
        <h1 className="text-4xl leading-tight font-bold md:text-5xl">Prettier의 이상한 구조</h1>
      ...
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold">마지막 플러그인만 적용된다</h2>
        <p className="text-muted-foreground leading-relaxed">
          Prettier에 플러그인을 여러개 적용시키면 가끔 앞쪽 플러그인이 무시되는 현상이 발생합니다. 때문에 몇몇
          플러그인은 플러그인을 적용할 때, 가장 마지막에 놓아달라고 요청하는 경우도 있습니다. 예를 들어{" "}
          <CodeBlock code={`"prettier-plugin-tailwindcss"`} variant="inline" lang="json" /> 의 경우에는 .prettierrc
          파일에 아래와 같이 설정하라고 안내하고 있습니다.
        </p>
        <CodeBlock code={TAILWINDCSS_PLUGIN_INSTRUCTION} lang="json" />
        <p className="text-muted-foreground leading-relaxed">
      ...
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold">모든 것은 플러그인</h2>
        <p className="text-muted-foreground leading-relaxed">
        ...
        <CodeBlock code={WITH_PLUGINS} lang="javascript" />
        <p className="text-muted-foreground leading-relaxed">
      ...
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold">플러그인 인터페이스와 동작</h2>
        <p className="text-muted-foreground leading-relaxed">
      ...
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold">Parser - 모든 문제의 시작</h2>
        <p className="text-muted-foreground leading-relaxed">
        ...
        <CodeBlock code={PLUGIN_LANGUAGE} />
        <p className="text-muted-foreground leading-relaxed">
        ...
        <p className="text-muted-foreground leading-relaxed">
          Prettier는 <CodeBlock code="Array.findLast()" variant="inline" /> 를 사용하기 때문에 가장 마지막 parser가
        ...
        <CodeBlock code={PRETTIER_PLUGINS_EXAMPLE} lang="json" />
        <p className="text-muted-foreground leading-relaxed">
          세 플러그인은 하는일은 다르지만 전부 estree parser를 사용합니다. 하지만 parser의 이름만 같을 뿐, 출력되는
          AST의 형식이 모두 다릅니다.{" "}
          <CodeBlock code={`"@xeonlink/prettier-plugin-organize-attributes"`} variant="inline" lang="json" /> 의
          parser만 실행되고, 그에 맞는 AST가 생성될 뿐입니다.
        </p>
      </section>

      <section className="space-y-4">
        <h2 className="text-2xl font-semibold">Printer — 이것도 똑같다.</h2>
        <p className="text-muted-foreground leading-relaxed">
          ...
          이용해서 printer를 찾습니다. printer도 마찬가지로 <CodeBlock code="Array.findLast()" variant="inline" /> 를
      ...
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold">코드를 분석하면서</h2>
        <p className="text-muted-foreground leading-relaxed">
          Prettier 코드를 분석하니, 왜 <CodeBlock code={`"prettier-plugin-tailwindcss"`} variant="inline" lang="json" />{" "}
          가 마지막에 와야하는 이유를 알 수 있었습니다. language와 parser, parser와 printer 사이의 연결을 위한 parser
          name, astFormat 값을 두는 것은 하나의 큰 모듈을 두지 않고 목적에 따라 코드를 분리하여 복잡도를 높이지 않기
          위한 선택이었을지도 모릅니다.
        </p>
        <p className="text-muted-foreground leading-relaxed">
